...
export const troubles: Trouble[] = [
  ...
  {
    slug: "game-score-recovery",
    title: "게임 점수 유실 복구 (재제출 설계)",
    tag: "레거시 · 점수 저장",
    summary:
      "저장에 실패한 점수를 다시 제출해 복구하되, 같은 판의 중복 제출은 여전히 막히도록 설계.",
    situation:
      "네트워크 끊김이나 앱 종료로 게임 점수 저장 요청이 유실되면, 유저가 실제로 획득한 점수가 사라지는 문제가 있었습니다. 랭킹·포인트 정산과 직결되는 데이터라 그대로 둘 수 없었습니다.",
    cause:
      "점수 저장이 게임 종료 시점의 1회 요청에만 의존했고, 실패한 기록을 다시 살릴 경로가 없었습니다. 단순히 재전송을 허용하면 이번엔 같은 점수의 중복 제출·조작 여지가 생기는 게 딜레마였습니다.",
    solution:
      "미기록 점수를 재제출하는 복구 경로(is_unrecorded)를 추가하되, 서버는 게임 시작 시 발급한 트랜잭션 id(tid)로 DB에 이미 저장된 기록인지 먼저 확인해 중복이면 조용히 성공 처리(멱등)했습니다. 정상 경로는 기존 tid 캐시 검증을 그대로 유지해, 복구 경로가 부정 제출 우회로가 되지 않게 분리했습니다.",
    result:
      "저장 실패로 점수가 사라지는 문제를 복구하면서도 중복·조작 제출은 여전히 차단됩니다. 복구(신뢰성)와 방어(무결성)는 서로 부딪히기 쉬운데, 멱등성 설계로 둘 다 지킬 수 있었습니다.",
  },
  {
    slug: "legacy-settlement-date",
    title: "일일 랭킹 정산 기록 누락 (데이터 무결성)",
    tag: "레거시 · 정산",
    summary:
      "자정 경계에서 게임 기록이 다음 날로 밀려 정산에서 누락되던 문제를 집계 기준 교체로 해결.",
    situation:
      "일일 랭킹 기반 포인트 정산을 운영하던 중, 일부 유저의 전날 게임 기록이 랭킹·정산에서 누락되는 문제가 발견됐습니다. 포인트가 실제로 지급되는 로직이라 그대로 둘 수 없었습니다.",
    cause:
      "랭킹 집계가 레코드 생성 시각(created_date) 기준이었는데, 자정 직전에 시작해 자정을 넘겨 저장된 게임은 생성 시각이 다음 날로 넘어가 전날 랭킹에서 빠졌습니다.",
    solution:
      "집계 기준을 게임 완료 시각(finished_date)으로 교체해, 게임을 마친 날짜 기준으로 랭킹이 계산되도록 바로잡았습니다. 재발 방지를 위해 관련 모델의 집계 쿼리를 함께 점검했습니다.",
    result:
      "자정 경계의 기록 누락이 사라져 정산이 다시 정확해졌습니다. 시작·저장·완료처럼 시각이 여러 개인 데이터는 어떤 시각을 집계 기준으로 삼느냐가 곧 정산의 정확성이라는 걸 배웠습니다.",
  },
];
